...
class Adafruit_Thermal(Serial):
    ...
    def __init__(self, port=None, *args, **kwargs):
        ...
        if self.writeToStdout is False:
            ...
            if "baudrate" in kwargs:
                ...
                self.byteTime = 11.0 / float(kwargs['baudrate'])
            else:
                self.byteTime = 11.0 / float(default_baudrate)

            if "baudrate" in kwargs:
                Serial.__init__(self, port, *args, **kwargs)
            else:
                Serial.__init__(self, port, default_baudrate, *args, **kwargs)
    ...
    def printBitmap(self, w, h, bitmap, LaaT=False):
        ...
        for rowStart in range(0, h, maxChunkHeight):
            ...
            for y in range(chunkHeight):
                if self.writeToStdout:
                    sys.stdout.write(bitmap[i:i + rowBytesClipped])
                else:
                    super(Adafruit_Thermal,
                          self).write(bitmap[i:i + rowBytesClipped])
                i += rowBytes
    ...
    def printImage(self, image, LaaT=False):
        if not isinstance(image, Image.Image):
            logging.error("printImage() didn't receive a PIL.Image.Image object!")
            return
