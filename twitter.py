...
import base64, html.parser, http.client, json, sys, urllib.request, urllib.parse, urllib.error, zlib
...
def issueRequestAndDecodeResponse(method, url, body, headers):
    ...
    if response.status != 200:
        ...
        # keep it commented out when using start_printer.py
...
token = issueRequestAndDecodeResponse(
    ...
    {'Host': host,
     ...
     'Authorization': 'Basic ' + base64.b64encode(
         urllib.parse.quote(consumer_key) + ':' + urllib.parse.quote(consumer_secret))}
)['access_token']
...
data = issueRequestAndDecodeResponse(
    ...
    (searchUrl + 'count=3&since_id=%s&q=%s' %
     (lastId, urllib.parse.quote(queryString))),
    None,
    {'Host': host,
     'User-Agent': agent,
     'Accept-Encoding': 'gzip',
...
for tweet in data['statuses']:
    ...
    printer.print(unidecode(
        html.parser.HTMLParser().unescape(tweet['text'])))
